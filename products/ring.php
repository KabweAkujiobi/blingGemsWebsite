...
if(isset ($_POST["add"])){
    ...
    if(isset($_SESSION["cart"])){
        ...
        if(!in_array($_GET["ring_id"], $item_array_id)){
            $count = count($_SESSION["cart"]);
            $item_array = array(
                'item_id'            => $_GET["ring_id"],
            ...
            echo '<script>window.location="ring.php"</script>';
         ...
         <h3>Rings</h3>
            ...
            if(mysqli_num_rows($result) > 0){
                while($row = mysqli_fetch_array($result)){
                    ...
                    <form method = "post" action="ring.php?action=add&ring_id=<?php echo $row["ring_id"]; ?>">
                    <img src="<?php echo $row["ring_image"];  ?>"/>
                    <h5> <?php echo $row["ring_name"] ?></h5>
                    <p>R <?php echo $row["ring_price"] ?></p>
                    <input type="hidden" name="hidden_name" value="<?php echo $row["ring_name"] ?>"/>
                    <input type="hidden" name="hidden_price" value="<?php echo $row["ring_price"] ?>"/>
